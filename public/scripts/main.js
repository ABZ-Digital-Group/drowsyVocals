...
// ==========================================================================
// ANTI-STREAM / SCREENSHOT DEFENSE SUITE
// ==========================================================================

// 1. AUTO-BLACKOUT PRIVACY CURTAIN ON LOSS OF WINDOW FOCUS
const securityCurtain = document.getElementById("securityCurtain");

const showSecurityCurtain = () => {
  if (securityCurtain) {
    securityCurtain.style.display = "flex";
  }
};

const hideSecurityCurtain = () => {
  if (securityCurtain) {
    securityCurtain.style.display = "none";
  }
};

// Activate privacy curtain when tab/window is blurred or tab is switched
window.addEventListener("blur", showSecurityCurtain);
document.addEventListener("visibilitychange", () => {
...
// Dismiss curtain when user clicks back into window or focuses it
window.addEventListener("focus", hideSecurityCurtain);
if (securityCurtain) {
  securityCurtain.addEventListener("click", hideSecurityCurtain);
}

// 2. STREAMER PRIVACY REDACTION MODE (TOGGLE & HOTKEY: ALT + P)
const streamModeToggle = document.getElementById("streamModeToggle");
const streamModeIcon = document.getElementById("streamModeIcon");

const setStreamMode = (enabled) => {
  if (enabled) {
    document.body.classList.add("stream-safe-mode");
    streamModeToggle?.classList.add("is-active");
    streamModeToggle?.setAttribute("aria-pressed", "true");
    if (streamModeIcon) streamModeIcon.textContent = "visibility_off";
    localStorage.setItem("drowsy_stream_safe", "true");
  } else {
    document.body.classList.remove("stream-safe-mode");
    streamModeToggle?.classList.remove("is-active");
    streamModeToggle?.setAttribute("aria-pressed", "false");
    if (streamModeIcon) streamModeIcon.textContent = "visibility";
    localStorage.removeItem("drowsy_stream_safe");
  }
};

// Initialize saved preference
if (localStorage.getItem("drowsy_stream_safe") === "true") {
  setStreamMode(true);
}

if (streamModeToggle) {
  streamModeToggle.addEventListener("click", () => {
    const isCurrentlyActive = document.body.classList.contains("stream-safe-mode");
    setStreamMode(!isCurrentlyActive);
  });
}
